...
function filterProducts(products, category, minPrice, maxPrice) {
    return products.filter(p => {
        return p.category === category && p.price >= minPrice && p.price <= maxPrice;
    });
}

// 3. Sắp xếp theo giá (tăng hoặc giảm)
...
// 4. Tìm sản phẩm rẻ nhất trong mỗi category
function cheapestByCategory(products) {
    // dùng reduce để gom nhóm
    let result = products.reduce((acc, product) => {
        let cat = product.category;
        // Nếu chưa có category này, hoặc sản phẩm này rẻ hơn thì cập nhật
...
// 5. Tính tổng giá trị kho hàng (price × stock)
...
// 6. Tạo mảng chỉ có { name, formattedPrice }
function formatProductList(products) {
    return products.map(p => {
        // Định dạng giá kiểu 25.990.000đ
        let formattedPrice = p.price.toLocaleString("vi-VN") + "đ";
        return { name: p.name, formattedPrice };
    });
}

// 7. Tính rating trung bình
function averageRating(products) {
    let total = products.reduce((sum, p) => sum + p.rating, 0);
